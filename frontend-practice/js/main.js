...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Three.js 3D Animation Setup - Wait for Three.js to load
    if (typeof THREE !== 'undefined') {
        try {
            ...
            console.error('❌ 3D animations failed to initialize:', error);
            showThreeJSFallback();
        }
    } else {
        console.warn('⚠️ Three.js not loaded, waiting...');
        // Wait for Three.js to load
        let threeCheckInterval = setInterval(() => {
            if (typeof THREE !== 'undefined') {
                ...
                try {
                    ...
                    setupThreeJSControls();
                    setupRoomControls();
                    console.log('✅ 3D animations initialized successfully (delayed)');
                } catch (error) {
                    console.error('❌ 3D animations failed to initialize (delayed):', error);
                    showThreeJSFallback();
                }
            }
        }, 100);
        ...
        setTimeout(() => {
            clearInterval(threeCheckInterval);
            if (typeof THREE === 'undefined') {
                console.error('❌ Three.js failed to load within 5 seconds');
                showThreeJSFallback();
            }
        }, 5000);
    }
...
function initThreeJS() {
    ...
    console.log('Animation variables:', { isAnimating, isMouseOver });
    console.log('Scene objects:', { scene: !!scene, camera: !!camera, renderer: !!renderer, thunderbolt: !!thunderbolt });
    console.log('Canvas info:', { 
        width: renderer.domElement.width, 
        height: renderer.domElement.height,
        style: renderer.domElement.style.cssText,
        inDOM: document.body.contains(renderer.domElement)
    });
    animate();
}

function animate() {
    // Ensure continuous animation loop
    requestAnimationFrame(animate);
    
    // Safety check - ensure all objects exist
    if (!scene || !camera || !renderer) {
        console.warn('Animation stopped - missing scene components');
        return;
    }
    
    if (isAnimating && !isMouseOver && thunderbolt) {
        // Slow, graceful rotation
        const time = Date.now() * 0.0003; // Much slower time multiplier
        thunderbolt.rotation.x += 0.02; // Faster for debugging
        thunderbolt.rotation.y += 0.03;
        thunderbolt.rotation.z += 0.015;
        
        // Debug: Log rotation every 5 seconds
        if (Math.floor(time) % 5 === 0 && Math.floor(time * 10) % 10 === 0) {
            console.log(`🔄 Thunderbolt rotating: x=${thunderbolt.rotation.x.toFixed(2)}, y=${thunderbolt.rotation.y.toFixed(2)}`);
        }
    ...
    if (Math.floor(Date.now() / 5000) !== Math.floor((Date.now() - 16) / 5000)) {
        console.log('🎨 Rendering thunderbolt scene...');
    }
...
function initRoomScene() {
    ...
    // Start animation loop
    console.log('🏢 Starting room animation loop...');
    console.log('Room objects:', { roomScene: !!roomScene, roomCamera: !!roomCamera, roomRenderer: !!roomRenderer });
    animateRoom();
}
...
function animateRoom() {
    ...
    // Safety check - ensure all objects exist
    if (!roomScene || !roomCamera || !roomRenderer) {
        console.warn('Room animation stopped - missing scene components');
        return;
    }
...
function initAISuggestions() {
    ...
    function createSuggestionElement(suggestion, index) {
        return `
            <div class="suggestion-item" style="animation-delay: ${index * 0.1}s">
                <div class="suggestion-icon"><div style="width: 52px; height: 52px; background-color: ${suggestion.icon.color}; border-radius: 8px; display: flex; align-items: center; justify-content: center; color: ${suggestion.icon.color === '#FFD700' || suggestion.icon.color === '#4ECDC4' || suggestion.icon.color === '#A8E6CF' || suggestion.icon.color === '#FFD93D' || suggestion.icon.color === '#F7DF1E' ? '#000000' : '#FFFFFF'}; font-weight: bold; font-size: 16px;">${suggestion.icon.text}</div></div>
    ...
    function showSuggestions(query) {
        ...
        if (suggestions.length === 0) {
            suggestions = [
                { title: 'Popular frontend topics', subtitle: 'Trending questions and tutorials', icon: { color: '#FF6B35', text: 'HOT' } },
                { title: 'Design inspiration', subtitle: 'Beautiful UI patterns and examples', icon: { color: '#A8E6CF', text: 'ART' } },
                { title: 'Code challenges', subtitle: 'Practice problems and solutions', icon: { color: '#FFD93D', text: 'CODE' } }
            ];
        }
...
function initCleanAISuggestions() {
    ...
    const suggestionSets = {
        'how': [
            { title: 'How to learn JavaScript?', subtitle: 'Programming fundamentals and best practices', icon: { color: '#FFD700', text: 'JS' } },
            { title: 'How to design better?', subtitle: 'UI/UX principles and design systems', icon: { color: '#FF6B6B', text: 'UI' } },
            { title: 'How to optimize performance?', subtitle: 'Web performance and optimization tips', icon: { color: '#4ECDC4', text: '⚡' } }
        ],
        'what': [
            { title: 'What is React?', subtitle: 'Modern JavaScript library for UI', icon: { color: '#61DAFB', text: '⚛' } },
            { title: 'What are design tokens?', subtitle: 'Scalable design system foundations', icon: { color: '#9B59B6', text: '🎯' } },
            { title: 'What is Three.js?', subtitle: '3D graphics library for the web', icon: { color: '#000000', text: '3D' } }
        ],
        'why': [
            { title: 'Why use TypeScript?', subtitle: 'Type safety and better experience', icon: { color: '#3178C6', text: 'TS' } },
            { title: 'Why design systems matter?', subtitle: 'Consistency and scalability benefits', icon: { color: '#27AE60', text: 'DS' } },
            { title: 'Why learn frontend?', subtitle: 'Career opportunities and creativity', icon: { color: '#F39C12', text: 'FE' } }
        ],
        'best': [
            { title: 'Best practices for CSS?', subtitle: 'Modern CSS techniques and patterns', icon: { color: '#1572B6', text: 'CSS' } },
            { title: 'Best UI animation libraries?', subtitle: 'Smooth and performant animations', icon: { color: '#E74C3C', text: 'AN' } },
            { title: 'Best learning resources?', subtitle: 'Curated tutorials and documentation', icon: { color: '#8E44AD', text: 'ED' } }
        ],
        'javascript': [
            { title: 'JavaScript ES6+ features', subtitle: 'Modern syntax and capabilities', icon: { color: '#F7DF1E', text: 'JS6' } },
            { title: 'JavaScript async/await', subtitle: 'Handling asynchronous operations', icon: { color: '#FF9500', text: 'AS' } },
            { title: 'JavaScript frameworks comparison', subtitle: 'React, Vue, Angular overview', icon: { color: '#2ECC71', text: 'FW' } }
        ],
        'css': [
            { title: 'CSS Grid vs Flexbox', subtitle: 'Layout techniques comparison', icon: { color: '#3498DB', text: 'GF' } },
            { title: 'CSS animations guide', subtitle: 'Keyframes and transitions', icon: { color: '#E67E22', text: 'AN' } },
            { title: 'CSS custom properties', subtitle: 'CSS variables and theming', icon: { color: '#9B59B6', text: 'CP' } }
        ]
    };
...
// Three.js Fallback Function
function showThreeJSFallback() {
    const thunderboltContainer = document.getElementById('threejs-container');
    const roomContainer = document.getElementById('room-container');
    
    if (thunderboltContainer) {
        thunderboltContainer.innerHTML = `
            <div style="display: flex; align-items: center; justify-content: center; height: 100%; color: #666; text-align: center; padding: 2rem; background: linear-gradient(135deg, #87ceeb, #b0e0e6);">
                <div>
                    <div style="font-size: 3rem; margin-bottom: 1rem;">⚡</div>
                    <h3 style="margin: 0; font-size: 1.2rem; color: #333;">3D Thunderbolt Animation</h3>
                    <p style="margin: 0.5rem 0 0 0; font-size: 0.9rem; opacity: 0.7;">WebGL or Three.js not available</p>
                </div>
            </div>
        `;
    }
    ...
    if (roomContainer) {
        roomContainer.innerHTML = `
            <div style="display: flex; align-items: center; justify-content: center; height: 100%; color: #666; text-align: center; padding: 2rem; background: linear-gradient(135deg, #f0f0f0, #e0e0e0);">
                <div>
                    <div style="font-size: 3rem; margin-bottom: 1rem;">🏢</div>
                    <h3 style="margin: 0; font-size: 1.2rem; color: #333;">3D Room Scene</h3>
                    <p style="margin: 0.5rem 0 0 0; font-size: 0.9rem; opacity: 0.7;">WebGL or Three.js not available</p>
                </div>
            </div>
        `;
